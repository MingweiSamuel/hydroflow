            ...
            if let Some(receivers) = weak_receivers.upgrade() {
                let send_tasks = {
                    ...
                    receivers
                        .iter()
                        .map(|receiver| {
                            // Create a future which has it's own `sender` and `line`, but do not run it yet.
                            let receiver = receiver.clone();
                            let line = line.clone();
                            async move { receiver.clone().send(line).await }
                        })
                        .collect::<Vec<_>>()
                    // We CANNOT `.await` while holding the `receivers` lock, so we collect tasks into a vec.
                    // `receivers` lock is dropped here.
                };

                let successful_send = join_all(send_tasks)
                    .await
                    .into_iter()
