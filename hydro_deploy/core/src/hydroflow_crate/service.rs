...
impl HydroflowCrateService {
    ...
    pub fn stdout(&self) -> Receiver<String> {
        self.launched_binary.as_deref().unwrap().stdout()
    }

    pub fn stderr(&self) -> Receiver<String> {
        self.launched_binary.as_deref().unwrap().stderr()
    }

    pub fn exit_code(&self) -> Option<i32> {
        self.launched_binary.as_deref().unwrap().exit_code()
    }
...
impl Service for HydroflowCrateService {
    ...
    async fn start(&mut self) -> Result<()> {
        ...
        let stdout_receiver = self.launched_binary.as_deref_mut().unwrap().cli_stdout();
