...
<CodeBlock language="rust" title="flow/src/first_ten.rs">{getLines(firstTenSrc, 'flowgraph')}</CodeBlock>

## The Flow Graph

In a new file, let's extend this example to print the numbers on a separate process. First, we need to specify that our flow graph will involve the network. We do this by replacing the `LocalDeploy<'a>` trait bound with the general `Deploy<'a>`. Then, we can use the `process_spec` to create a second process.

<CodeBlock language="rust" title="flow/src/first_ten_distributed.rs">{getLines(firstTenDistSrc, 'flowgraph_start')}</CodeBlock>
...
<CodeBlock language="rust">{getLines(firstTenDistSrc, 'flowgraph_numbers')}</CodeBlock>

Finally, we return the handle for the second process.

<CodeBlock language="rust">{getLines(firstTenDistSrc, 'flowgraph_return')}</CodeBlock>
...
<CodeBlock language="rust" title="flow/src/first_ten_distributed.rs">{getLines(firstTenDistSrc, 'runtime')}</CodeBlock>
