...
fn operator_docgen_internal(item: &ItemConst) -> Result<(), Box<dyn Error>> {
    ...
    writeln!(
        ...
        Span::call_site().unwrap().source_file().path()
    )?;
    ...
    for attr in item.attrs.iter() {
        ...
        if doc_str.trim_start().starts_with("```") {
            if in_hf_doctest {
                in_hf_doctest = false;
                writeln!(docgen_write, "# }};")?;
                writeln!(docgen_write, "# __hf.run_available();")?;
                writeln!(docgen_write, "{}", doc_str)?;
            } else if doc_str.trim() == "```hydroflow" {
                in_hf_doctest = true;
                // writeln!(doctest_write, "#[test]")?;
                // writeln!(doctest_write, "fn doctest_{}() {{", counter.next().unwrap())?;
...
// struct ReplaceHydroflowDocTests {
//     inside_doctest: bool,
// }
// impl VisitMut for ReplaceHydroflowDocTests {
