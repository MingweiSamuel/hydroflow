...
pub type MapUnionVec<K, Val> = MapUnion<Vec<(K, Val)>>;

/// Array-backed [`MapUnion`] lattice.
pub type MapUnionArray<K, Val, const N: usize> = MapUnion<Array<(K, Val), N>>;

/// [`crate::collections::MaskedArray`]-backed [`MapUnion`] lattice.
pub type MapUnionMaskedArray<K, Val, const N: usize> = MapUnion<MaskedArray<(K, Val), N>>;

/// [`crate::collections::Single`]-backed [`MapUnion`] lattice.
pub type MapUnionSingle<K, Val> = MapUnion<Single<(K, Val)>>;
