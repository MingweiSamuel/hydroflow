...
The `lattices` crate provides traits and implementations for semilattice data types. Semilattices
are the mathematical concept underyling CRDTs;
their useful properties (associativity, commutativity, and idempotence) allow data updates to run
in any order while still preserving the same end result.

## Lattices

`lattices` provides implementation of common lattice types:
* [`Min<T>`] and [`Max<T>`]
* [`set_union::SetUnion`]
