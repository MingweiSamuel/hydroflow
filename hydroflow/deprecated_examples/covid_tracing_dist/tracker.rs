...
pub(crate) async fn run_tracker(opts: Opts) {
    ...
    df.add_subgraph(
        "network demux",
        var!(network_in),
        var!(contacts, diagnoses),
        move |_ctx, var!(recv), var!(send1, send2)| {
    ...
    df.add_subgraph(
        "main",
        var!(contacts_in, diagnosed_in, loop_in),
        var!(notifs_out, loop_out),
        move |_ctx, var!(contacts_recv, diagnosed_recv, loop_recv), var!(notifs_send, loop_send)| {
