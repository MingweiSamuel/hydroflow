...
pub struct Hydroflow {
    ...
    /// If external events or data can justify starting the next tick (or current tick if we are on
    /// stratum 0).
    can_start_tick: bool,
...
impl Hydroflow {
    ...
    /// If `current_tick_only` is set to `false`, will only continue to the next tick if meaningful
    /// work (external or data-triggered) is available.
    ///
