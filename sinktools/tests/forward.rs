...
#[tokio::test]
async fn test_forward_vs_backward_equivalence() {
    // Test that forward and backward chaining produce the same results
    ...
    // Backward chaining (equivalent)
    let mut backward_pipeline = backward_sink
        .un_filter_map(|x: i32| if x < 20 { Some(x + 1) } else { None })
        .un_map(|x| x * 3)
        .un_filter(|x: &i32| *x % 2 == 0);
    ...
    assert_eq!(&**forward_collected.borrow(), &**backward_collected.borrow());
